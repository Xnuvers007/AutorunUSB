package dev.xnuvers007.usbautorun;
...
public class USBAutorunCreator {
    ...
    private static void createAutorun(String driveLetter, String name, String apps) {
        try {
            ...
            // get filename
            Path iconPath = Paths.get(name);
            String iconName = iconPath.getFileName().toString();

            Path appPath = Paths.get(apps);
            ...
            if (Arrays.asList("FAT32", "NTFS", "exFAT").contains(fileSystem.toUpperCase())) {
                ...
                List<String> autorunContent = Arrays.asList(
                    ...
                    "shell\\open\\command=" + appName,
                    "shell\\explore\\command=" + appName,
                    "shell\\open=" + appName,
                    "shell\\explore=" + appName,
                    "shell\\open\\default=" + appName,
                    "shell\\explore\\default=" + appName,
                    "UseAutoPlay=1",
                    "UseAutoRun=1"
                );
                ...
                // Copy icon
                Path iconSource = Paths.get(name);
                Path iconDest = Paths.get(driveLetter, iconSource.getFileName().toString());
                ...
                // Copy app
                Path appSource = Paths.get(apps);
                Path appDest = Paths.get(driveLetter, appSource.getFileName().toString());
                ...
                try {
                    ...
                    if (hideAutorun.equals("y")) {
                        ...
                        processBuilder.command("cmd.exe", "/c", "attrib +h +s " + autorunPath.toString());
                        processBuilder.start();
                    }

                    if (hideFiles.equals("y")) {
                        ProcessBuilder processBuilder = new ProcessBuilder();
                        processBuilder.command("cmd.exe", "/c", "attrib +h +s " + iconDest.toString());
                        processBuilder.start();
                        processBuilder.command("cmd.exe", "/c", "attrib +h +s " + appDest.toString());
                        processBuilder.start();
                    }
